...
# if statement needed to fool Sphinx
if str(sys.argv[2]) == "html":
    pass
else:
    pdb_file = str(sys.argv[2])
    # Can easily modify to write to file directly
    # out_file = "2fd7_new.xyzr"
    #solvent = int(float(sys.argv[1]))
    ###
    if str(sys.argv[1]) != "-h":
        try:
            solvent = int(float(sys.argv[1]))
        except ValueError:
            print("First argument not a valid number")
    else:
            try:
                solvent = int(float(sys.argv[2]))
            except ValueError:
                print("Second argument not a valid number")
