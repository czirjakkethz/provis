...
def pdb_to_xyzr_script(name, solv, bash=0):
    ...
    if not bash:
        ac = subprocess.call("python pdb_to_xyzr.py %1s %4s.pdb > %4s.xyzr" % (str(int(solv)), name, name), shell=True)
    else:
        ac = subprocess.call("./pdb_to_xyzr %4s.pdb > %4s.xyzr" % (name, name), shell=True)
